const API_BASE =
  import.meta.env.VITE_API_BASE || "http://localhost:8000"; // Fallback für lokale Entwicklung
const JSON_HEADERS = { "Content-Type": "application/json" };

async function req(method, path, body) {
  const url = `${API_BASE}${path}`; // <-- hier wird deine App Runner URL automatisch ergänzt
...
export const createWorldItem = (pid, payload) =>
  req("POST", `/api/projects/${pid}/world-items`, payload);
export const updateWorldItem = (id, payload) =>
  req("PUT", `/api/world-items/${id}`, payload);
export const deleteWorldItem = (id) =>
  req("DELETE", `/api/world-items/${id}`);
